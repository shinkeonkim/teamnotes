\documentclass[10pt,landscape,a4paper,twocolumn]{article}

\setlength{\columnsep}{20pt}

\usepackage[left=1.0cm, right=1.0cm, top=1.5cm, bottom=1.0cm, headsep=0.4cm]{geometry}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{fontspec}
\usepackage{kotex}
\usepackage{graphicx}
\usepackage{setspace}
\usepackage{listings}
\usepackage{comment}
\usepackage{import}
\usepackage{wrapfig}
\usepackage{url}
\usepackage{array}
\usepackage[normal]{engord}
\usepackage[svgnames,table]{xcolor}

\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhead[R]{\thepage}
\fancyhead[L]{휴리스틱 원툴팀}

\setmonofont[
    BoldFont = consolab.ttf,
    ItalicFont = consolai.ttf
]{consola.ttf}
\setmainhangulfont{NanumMyeongjo}
\setlength\parindent{0pt}
\usepackage[parfill]{parskip}

\definecolor{dkgrey}{RGB}{127, 127, 127}

\lstset{
    basicstyle=\footnotesize\ttfamily,
    breaklines=true,
    breakindent=1.1em
%    numbers=left,
%    numberstyle=\footnotesize\ttfamily\color{dkgrey},
%    numbersep=5pt
%    frame=trbl
}

\lstdefinestyle{mycpp}{
  language=[GNU]C++,
  keywordstyle=\color{blue},
  commentstyle=\itshape\color{purple!40!black},
  stringstyle=\color{orange},
}

\begin{document}
\tableofcontents


\section{Setting}
\subsection{PS}
\lstinputlisting[style=mycpp]{src/setting/ps.cpp}




\section{Math}
\subsection{Basic Arithmetics}
\lstinputlisting[style=mycpp]{src/math/basic-arithmetics.cpp}

\subsection{Convex Hull Trick}
\lstinputlisting[style=mycpp]{src/math/convex-hull-trick-linear.cpp}

\subsection{Luca Theorem}
\lstinputlisting[style=mycpp]{src/math/lucas-theorem.cpp}

\subsection{Pollard Rho}
\lstinputlisting[style=mycpp]{src/math/pollard-rho.cpp}

\subsection{Gaussian Elimination}
\lstinputlisting[style=mycpp]{src/math/gaussian.cpp}

\subsection{Sieve}
\lstinputlisting[style=mycpp]{src/math/sieve.cpp}



\subsection{FFT}
\lstinputlisting[style=mycpp]{src/math/fft.cpp}

\subsection{Chinese Remainder}
\lstinputlisting[style=mycpp]{src/math/chinese-remainder.cpp}



\section{Data Structure}
\subsection{Fenwick Tree}
\lstinputlisting[style=mycpp]{src/data-structure/fenwick-tree.cpp}

\subsection{Fenwick Tree 2D}
\lstinputlisting[style=mycpp]{src/data-structure/fenwick-tree-2d.cpp}

\subsection{Merge Sort Tree}
\lstinputlisting[style=mycpp]{src/data-structure/merge-sort-tree.cpp}

\subsection{SegmentTree Lazy Propagation}
\lstinputlisting[style=mycpp]{src/data-structure/segtree-lazyprop.cpp}

\subsection{Treap}
\lstinputlisting[style=mycpp]{src/data-structure/treap.cpp}


\section{Geometry}
\subsection{Basic Operations}
\lstinputlisting[style=mycpp]{src/geometry/basic-operations.cpp}

\subsection{Convex Hull}
\lstinputlisting[style=mycpp]{src/geometry/convex-hull.cpp}

\subsection{Poiont in Polygon}
\lstinputlisting[style=mycpp]{src/geometry/point-in-polygon.cpp}

\subsection{Polygon Cut}
\lstinputlisting[style=mycpp]{src/geometry/polygon-cut.cpp}

\subsection{Rotating Calipers}
\lstinputlisting[style=mycpp]{src/geometry/rotating-calipers.cpp}

\subsection{Seperating Axis Theorem}
\lstinputlisting[style=mycpp]{src/geometry/seperating-axis-theorem.cpp}


% \subsection{Vector2}
% \lstinputlisting[style=mycpp]{src/geometry/vector2.cpp}

\subsection{Two Far Point}
\lstinputlisting[style=mycpp]{src/geometry/two-far-point.cpp}

\subsection{Two Nearest Point}
\lstinputlisting[style=mycpp]{src/geometry/two-near-point.cpp}


\section{Graph}

\subsection{Dijkstra}
\lstinputlisting[style=mycpp]{src/graph/dijkstra.cpp}

\subsection{Bellman-Ford}
\lstinputlisting[style=mycpp]{src/graph/bellman-ford.cpp}

\subsection{Spfa}
\lstinputlisting[style=mycpp]{src/graph/spfa.cpp}

\subsection{Topological Sort}
\lstinputlisting[style=mycpp]{src/graph/topological-sort.cpp}

\subsection{Strongly Connected Component}
\lstinputlisting[style=mycpp]{src/graph/scc.cpp}

\subsection{2-SAT}
\lstinputlisting[style=mycpp]{src/graph/2-sat.cpp}

\subsection{Union Find}
\lstinputlisting[style=mycpp]{src/graph/union-find.cpp}

\subsection{MST Prim}
\lstinputlisting[style=mycpp]{src/graph/mst-prim.cpp}

\subsection{Lowest Common Ancestor}
\lstinputlisting[style=mycpp]{src/graph/lca.cpp}


\subsection{Maxflow dinic}
\lstinputlisting[style=mycpp]{src/graph/maxflow-dinic.cpp}

\subsection{Maxflow Edmonds-Karp}
\lstinputlisting[style=mycpp]{src/graph/maxflow-ed.cpp}

\subsection{MCMF SPFA}
\lstinputlisting[style=mycpp]{src/graph/mcmf-spfa.cpp}

\subsection{MCMF}
\lstinputlisting[style=mycpp]{src/graph/mcmf.cpp}


\section{String}
\subsection{KMP}
\lstinputlisting[style=mycpp]{src/string/kmp.cpp}

\subsection{Manacher}
\lstinputlisting[style=mycpp]{src/string/manacher.cpp}

\subsection{Suffix Array}
\lstinputlisting[style=mycpp]{src/string/suffix-array-lcp.cpp}

\subsection{2nd Suffix Array}
\lstinputlisting[style=mycpp]{src/string/suffix-array.cpp}

\subsection{Trie}
\lstinputlisting[style=mycpp]{src/string/trie.cpp}

\subsection{Aho-Corasick}
\lstinputlisting[style=mycpp]{src/string/aho-corasick.cpp}

\subsection{Z Algorithm}
\lstinputlisting[style=mycpp]{src/string/z.cpp}

\section {Dynamic Programming}

\subsection{LIS}
\lstinputlisting[style=mycpp]{src/dp/lis.cpp}

\subsection{LIS only length}
\lstinputlisting[style=mycpp]{src/dp/lis-only-length.cpp}

\subsection{KnapSack}
\lstinputlisting[style=mycpp]{src/dp/knapsack.cpp}

\subsection{Coin Change}
\lstinputlisting[style=mycpp]{src/dp/coin-change.cpp}

\subsection{Bit Field DP}
\lstinputlisting[style=mycpp]{src/dp/bit-field-dp.cpp}

\subsection{Knuth Optimization}
\lstinputlisting[style=mycpp]{src/dp/knuth-optimization.cpp}

\end{document}